...
node {
    ...
    def SF_CONSUMER_KEY='3MVG9pRzvMkjMb6lo8vCHgGoDZiG3_n5oNi.qmWkHF8WhPu3K3nnoum0Pf7F6yjNlAma7ZCTwCih2lTM66ymh'
    def SF_USERNAME='assignment@vamritech.com'
    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Authorize DevHub') {
                rc = command "${toolbelt}/sfdx force:auth:jwt:grant --instanceurl ${SF_INSTANCE_URL} --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwt-key-file ${server_key_file} --setdefaultdevhubusername --setalias HubOrg"
                if (rc != 0) {
